#!/usr/bin/env python3
"""
Fix SCM URLs in all batches using extended corrections
"""
...
from scm_corrections_extended import get_scm_info
...
def fix_batch_file(batch_file: Path):
    ...
    def fix_build(match):
        ...
        if scm_info:
            ...
            # Also fix buildType if needed (auth0 should be MVN not GRADLE)
            if 'auth0' in group_id and build_type == 'GRADLE':
                build_type = 'MVN'
            
            if new_scm_url != old_scm_url or new_scm_rev != old_scm_rev or new_project != project:
                fixes_made += 1
