...
deps = [l.rstrip() for l in deps_file.read_text().splitlines() if l.strip()]
...
for dep in deps:
    ...
    if '.redhat-' in v or '-redhat-' in v:
        prod.append(normalized)
        prod_det.append(f'{normalized} -> already productized in source list')
    else:
        unprod.append(normalized)
        unprod_det.append(f'{normalized} -> no redhat suffix in source list')

groups = collections.Counter(d.split(':', 1)[0] for d in deps if ':' in d)
...
**Generated**: {generated}
**Analysis Scope**: Camel component-declared third-party compile dependencies from Camel 4.22.0
...
This report is based on the aggregated **third-party compile dependencies declared directly by Camel component POMs**.

1. Enumerated Camel components from the Camel BOM.
2. Downloaded each component POM and extracted third-party compile dependencies.
3. Aggregated the unique dependency set across all components.
4. Classified dependencies locally:
   - versions already containing `redhat-` are treated as productized
   - versions without a Red Hat suffix are treated as pending productization
...
- **{productized_count}** already carry a Red Hat suffix in the source list.
- **{unproductized_count}** do not carry a Red Hat suffix and remain pending.
